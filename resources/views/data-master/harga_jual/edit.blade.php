...
<div class="container">
    ...
    <form action="{{ route('harga-jual.update', $hargaJual->id_harga) }}" method="POST">
        @csrf @method('PUT')
        ...
        <div class="mb-3">
            <label>Tanggal</label>
            <input type="date" name="tanggal" class="form-control" value="{{ $hargaJual->tanggal }}" required>
        </div>

        <div class="mb-3">
            <label>Harga</label>
            <input type="number" step="0.01" name="harga" class="form-control" value="{{ $hargaJual->harga }}" required>
        </div>
